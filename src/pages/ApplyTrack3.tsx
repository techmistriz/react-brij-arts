...
const stage1Schema = z.object({
  ...
  residentialConfirm: z.boolean().refine((v) => v, "You must confirm availability"),
  festivalConfirm: z.boolean().refine((v) => v, "You must confirm availability"),
  practiceDescription: z.string().min(1, "Required").refine((v) => countWords(v) <= 150, "Maximum 150 words"),
  practiceQuestion: z.string().min(1, "Required").refine((v) => countWords(v) <= 200, "Maximum 200 words"),
});
...
const disciplines = [
  "Visual Arts", "Performing Arts", "Music", "Craft",
  "Film & Moving Image", "Writing & Literature", "Curatorial Practice",
...
const ApplyTrack3 = () => {
  ...
  const s1 = useForm<Stage1Data>({
    ...
    defaultValues: {
      firstName: "", lastName: "", pronouns: "", country: "", city: "",
      currentRole: "", organisation: "", primaryDiscipline: "", yearsInCulture: "",
      email: "", phone: "", mailingAddress: "",
      residentialConfirm: false, festivalConfirm: false,
      practiceDescription: "", practiceQuestion: "",
    },
  });
    ...
    try {
      const { data: row, error } = await supabase
        ...
        .insert({
          ...
          residential_confirmed: data.residentialConfirm,
          festival_confirmed: data.festivalConfirm,
  ...
  const fieldError = (form: any, name: string) =>
    form.formState.errors[name] ? (
      <p className="text-destructive text-xs mt-1">{form.formState.errors[name]?.message as string}</p>
    ) : null;
      ...
      <main className="flex-1 pt-24">
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 py-12 md:py-20">
          <div className="grid lg:grid-cols-[1fr_1.4fr] gap-12 lg:gap-20">
            ...
            >
              <Link to="/apply" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
              ...
              <div className="flex gap-3">
                <div className={`flex-1 h-1.5 rounded ${stage >= 1 ? "bg-primary" : "bg-muted"}`} />
                <div className={`flex-1 h-1.5 rounded ${stage >= 2 ? "bg-primary" : "bg-muted"}`} />
              </div>
              ...
              <div className="border-t border-border pt-6">
                <p className="text-xs text-muted-foreground leading-relaxed">
                  <strong>Note:</strong> Selection is at the discretion of the review committee.
            ...
            >
              {stage === 1 ? (
                /* ════════ STAGE 1 ════════ */
                <form onSubmit={s1.handleSubmit(onStage1Submit)} className="space-y-12">
                  {/* Section A — About You */}
                  <div>
                    ...
                    <div className="space-y-5">
                      <div className="grid sm:grid-cols-2 gap-5">
                        <div>
                          ...
                          <Input {...s1.register("firstName")} className="mt-1.5" />
                          {fieldError(s1, "firstName")}
                        </div>
                        <div>
                          <Label>Last Name *</Label>
                          <Input {...s1.register("lastName")} className="mt-1.5" />
                          {fieldError(s1, "lastName")}
                        </div>
                      </div>
                      <div>
                        <Label>Pronouns (optional)</Label>
                        <Select onValueChange={(v) => s1.setValue("pronouns", v)}>
                      ...
                      <div>
                        ...
                        <Select onValueChange={(v) => s1.setValue("country", v)}>
                          <SelectTrigger className="mt-1.5">
                      ...
                      <div>
                        ...
                        <Input {...s1.register("currentRole")} className="mt-1.5" />
                      ...
                      <div>
                        ...
                        <Select onValueChange={(v) => s1.setValue("primaryDiscipline", v)}>
                          <SelectTrigger className="mt-1.5">
                      ...
                      <div>
                        ...
                        <Select onValueChange={(v) => s1.setValue("yearsInCulture", v)}>
                          <SelectTrigger className="mt-1.5">
                      ...
                      <div>
                        ...
                        <Input type="tel" {...s1.register("phone")} className="mt-1.5" placeholder="+91" />
                        {fieldError(s1, "phone")}
                      </div>
                      <div>
                        <Label>Mailing Address for Certificate *</Label>
                        <WordCountTextarea
                          {...s1.register("mailingAddress")}
                          rows={3}
                          placeholder="Full address including city and post code"
                          maxWords={100}
                          className="mt-1.5"
                        />
                        {fieldError(s1, "mailingAddress")}
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-start gap-3">
                        <Checkbox
                          id="residential"
                          checked={s1.watch("residentialConfirm")}
                          onCheckedChange={(v) => s1.setValue("residentialConfirm", !!v, { shouldValidate: true })}
                          className="mt-0.5"
                        />
                      ...
                      <div className="flex items-start gap-3">
                        <Checkbox
                          id="festival"
                          checked={s1.watch("festivalConfirm")}
                          onCheckedChange={(v) => s1.setValue("festivalConfirm", !!v, { shouldValidate: true })}
                          className="mt-0.5"
                        />
                        <label htmlFor="festival" className="text-sm cursor-pointer">
                          I can attend the Serendipity Arts Festival, Goa, December 2026 *
                        </label>
                      </div>
                      {fieldError(s1, "festivalConfirm")}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-6">
                      <div>
                        <Label>Describe your practice and the cultural context you work in. *</Label>
                        <WordCountTextarea
                          {...s1.register("practiceDescription")}
                          rows={6}
                          placeholder="Describe your practice..."
                          maxWords={150}
                          className="mt-1.5"
                        />
                        {fieldError(s1, "practiceDescription")}
                      </div>
                      <div>
                        <Label>
                          What question is your practice currently circling — something you don't yet
                          have clear language for? *
                        </Label>
                        <WordCountTextarea
                          {...s1.register("practiceQuestion")}
                          rows={6}
                          placeholder="Write your question here..."
                          maxWords={200}
                          className="mt-1.5"
                        />
                        {fieldError(s1, "practiceQuestion")}
                      </div>
                    </div>
                  </div>
                ...
                <form onSubmit={s2.handleSubmit(onStage2Submit)} className="space-y-12">
                  ...
                  <div>
                    ...
                    <div className="space-y-6">
                      <div>
                        <Label>
                          Describe a decision or turning point in your work in the last three years
                          that changed how you think. *
                        </Label>
                        <WordCountTextarea
                          {...s2.register("turningPoint")}
                          rows={6}
                          maxWords={200}
                          className="mt-1.5"
                        />
                        {fieldError(s2, "turningPoint")}
                      </div>
                      <div>
                        <Label>
                          What makes this a pivotal nine months for you? What do you hope the
                          Fellowship will make possible? *
                        </Label>
                        <WordCountTextarea
                  ...
                  <div>
                    ...
                    <div className="space-y-5">
                      <div>
                        ...
                        <Input
                          {...s2.register("workSampleUrl")}
                          placeholder="Google Drive link or URL (max 5 MB)"
                          className="mt-1.5"
                        />
                        {fieldError(s2, "workSampleUrl")}
                      </div>
                      <div>
                        <Label>In one sentence, tell us what you've shared and why you chose it. *</Label>
                        <WordCountTextarea
                          {...s2.register("workSampleDescription")}
                          rows={2}
                          maxWords={25}
                          className="mt-1.5"
                        />
                        {fieldError(s2, "workSampleDescription")}
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-start gap-3">
                      <Checkbox
                        id="timeCommit"
                        checked={s2.watch("timeCommitmentConfirm")}
                        onCheckedChange={(v) => s2.setValue("timeCommitmentConfirm", !!v, { shouldValidate: true })}
                        className="mt-0.5"
                      />
                      <label htmlFor="timeCommit" className="text-sm cursor-pointer leading-relaxed">
                        The Fellowship requires a maximum of 6 hours per week of structured engagement
                        across nine months, and two compulsory in-person moments in Goa. I have read
                        and understood this commitment. *
                      </label>
                    </div>
                  ...
                  <div className="flex gap-4 pt-4">
                    <button
                      type="button"
                      onClick={() => { setStage(1); window.scrollTo(0, 0); }}
